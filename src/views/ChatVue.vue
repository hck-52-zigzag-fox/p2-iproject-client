...
export default {
  ...
  computed: {
    ...
    getNameFromDataChat() {
      return this.dataChat.map((item) => {
        if (+item.dataReceiver.UserId == +this.$route.params.ReceiverId) {
          return item.dataReceiver.name;
        }
      })[0];
    },
  },
  methods: {
    ...
    fetchAll() {
      this.fetchChat(this.$route.params.ReceiverId);
      this.fetchAllProfile();
      this.fetchOneProfile(this.$route.params.ReceiverId);
    },
  },
  created() {
    this.fetchAll();
  },
  components: { ChatComponent, ContactCard },
};
</script>
...
<template>
  <div class="center">
    <div class="contacts">
      ...
      <h2>Contacts</h2>
      <ContactCard
        v-for="profile in dataAllProfiles"
        :key="profile.id"
        :profile="profile"
      />
    </div>
    <div class="chat">
      <div class="contact bar">
        <div class="pic">
          <img id="img" :src="dataProfileChat.profilePict" alt="" />
        </div>

        <div class="name">{{ dataProfileChat.name }}</div>
      </div>
      <div class="messages" id="chat">
        <ChatComponent v-for="chat in dataChat" :key="chat.id" :chat="chat" />
      </div>
      <form action="">
        <div class="input">
          ...
          ><input
            ...
            type="text"
          />
          <a @click.prevent="handleSendChat(dataMessage)" href=""
