  ...
  computed: {
    message() {
      if (!this.type) {
        ...
        return `Sorry we couldn't find ${this.type} you are looking for.`;
      }
    },
    codeMessage() {
      if (!this.type) {
...
<template>
  <section id="not-found" style="overflow-x: scroll; min-height: calc(100vh - 50px - 52.5px)">
    <div id="title">Awww.. &bull; Sorry</div>
    <div class="circles">
      <p>
        {{ codeMessage }}<br />
        <small>{{ message }}</small>
